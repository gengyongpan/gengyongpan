...
var exportColor = 0x44B2D4;
var importColor = 0x1658B2;
...
//	for svg loading
//	deprecated, not using svg load,ing anymore
var assetList = [];
                            ...
                            function(){
                                loadContentData(
                                    function(){
                                        initScene();
                                        animate();
                                        document.getElementById('history').style.left='15%';
                                    }
                                );
                            }
                        );
...
// 			specular: 	0x333333,
    ...
    buildGUI();
    //selectVisualization( timeBins, '2010', ['CHINA'], ['Military Weapons','Civilian Weapons', 'Ammunition'], ['Military Weapons','Civilian Weapons', 'Ammunition'] );
    selectVisualization( timeBins, (new Date()).getFullYear(), ['中国'], ['Military Weapons','Civilian Weapons', 'Ammunition'], ['Military Weapons','Civilian Weapons', 'Ammunition'] );
...
function getPickColor(){
    ...
    for(var i in buf) {
        ...
        if(i <3) {
            ...
            var alpha = ( d / 255);
            buf_index = Math.round(buf_index / (0.2 + 0.6 + 1) / alpha);
        }
    }
    return buf_index;
}